...
- Configure [content protection policies and DRM settings](gridwich-content-protection-drm).
- Run the [admin scripts](run-admin-scripts).
- Manage [Key Vault keys](maintain-keys).
- Set up a [local development environment](set-up-local-environment).
- Create or delete a [cloud sandbox or test environment](create-delete-cloud-environment).
- Follow [Test Azure Media Services v3 encoding](test-encoding) to fully test the setup.
- Scale [Media Services resources](media-services-setup-scale#scale-media-services-resources).
